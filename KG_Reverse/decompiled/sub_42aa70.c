...
void* sub_42aa70(void) {
  ...
  // 保存寄存器 eax
  // 写入栈变量 [esp+0x2c] = ecx
  // 通过分发表地址 0x4a2048 间接调用
  通过分发表间接调用 [0x4a2048]);
  /* 代码块 28 @ 0x0042AC97 */
  // 写入栈变量 [esp+0x14c] = ebp
  // 写入栈变量 [esp+0x148] = 0x2f96418
  // 计算栈偏移地址 14 -> ecx
  // mov byte ptr [esp + 0x284], 3
  // 保存寄存器 ecx
  // 通过分发表地址 0x4a209c 间接调用
  通过分发表间接调用 [0x4a209c]);
  /* 代码块 29 @ 0x0042ACBC */
  // 保存寄存器 eax
  // 计算栈偏移地址 14c -> ecx
