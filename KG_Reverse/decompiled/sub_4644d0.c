...
void* sub_4644d0(void) {
  ...
  // 保存寄存器 0
  // 保存寄存器 0xf001f
  // 通过分发表地址 0x4a2178 间接调用
  通过分发表间接调用 [0x4a2178]);
  /* 代码块 8 @ 0x0046452C */
  // 测试 eax 是否为零（设置标志位）
  // 写入全局变量 0x2fe8d48 = eax
  // 如果相等则跳转到 0x464551
  /* 代码块 9 @ 0x00464551 */
  // 计算栈偏移地址 c -> edx
  // 保存寄存器 edx
  ...
  // 如果不相等则跳转到 0x464717
  /* 代码块 19 @ 0x004645AB */
  // 写入全局变量 0x2fe8d5c = eax
  // 读取全局变量值 [edi + 4]
  // 写入全局变量 0x2fe8d50 = eax
  // mov ecx, dword ptr [edi + 8]
  // 写入全局变量 0x2fe8d54 = ecx
  // mov edx, dword ptr [edi + 0xc]
  // 写入全局变量 0x2fe8d58 = edx
  // 读取全局变量值 [edi + 0x10]
  // 写入全局变量 0x2fe8d44 = eax
  goto 0x46476f;
  /* 代码块 20 @ 0x004645E4 */
  // 从栈读取参数 0x218 到 ecx
  ...
  // 保存寄存器 0x2fd23a0
  // 写入全局变量 0x2fe8d5c = 1
  // 写入全局变量 0x2fe8d50 = esi
  // 写入全局变量 0x2fe8d54 = eax
  // 写入全局变量 0x2fe8d58 = ecx
  // call esi
  call(esi);
  /* 代码块 23 @ 0x004645EF */
  ...
  // 保存寄存器 ecx
  // 写入全局变量 0x2fe8d5c = 2
  // 写入全局变量 0x2fe8d50 = 0x463980
  // 写入全局变量 0x2fe8d54 = 0x4642b0
  // 写入全局变量 0x2fe8d58 = 0x464430
  // 直接调用函数 0x00463980
  call(0x00463980);
  /* 代码块 29 @ 0x004696CA */
  // mov edx, dword ptr [0x2fe8d4c]
  // 保存寄存器 edx
  // 通过分发表地址 0x4a21a8 间接调用
  通过分发表间接调用 [0x4a21a8]);
  /* 代码块 30 @ 0x00464783 */
  // mov edx, dword ptr [0x2fe8d44]
  // 保存寄存器 0x2fd2330
  // 保存寄存器 edx
  // 写入全局变量 0x2fe9b54 = eax
  // 通过分发表地址 0x2fe8d54 间接调用
  通过分发表间接调用 [0x2fe8d54]);
  /* 代码块 31 @ 0x00464697 */
  // 保存寄存器 0x2fd238c
